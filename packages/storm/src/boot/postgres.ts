...
export default function boot(ctx: Framework) {
  ...
  ctx.on('system-store-collection-insert', async (req, res) => {
    ...
    const store = ctx.plugin('storm-postgres').get(req.params.store);
  ...
  ctx.on('system-store-schema-alter', async (req, res) => {
    ...
    const builder = Query.alter(table, '"');
  ...
  ctx.plugin('storm-postgres', {
    add: (name: string, connection: Connection) => {
